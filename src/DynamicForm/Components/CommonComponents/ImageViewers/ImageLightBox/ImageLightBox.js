...
import { translate, getWindowWidth, getWindowHeight, getHighestSafeWindowContext } from './LightBoxUtil';
import { KEYS, MIN_ZOOM_LEVEL, MAX_ZOOM_LEVEL, ZOOM_RATIO, WHEEL_MOVE_X_THRESHOLD, WHEEL_MOVE_Y_THRESHOLD, ZOOM_BUTTON_INCREMENT_SIZE, ACTION_NONE, ACTION_MOVE, ACTION_SWIPE, ACTION_PINCH, SOURCE_ANY, SOURCE_MOUSE, SOURCE_TOUCH, SOURCE_POINTER, MIN_SWIPE_DISTANCE, ROTATIONMIN, ROTATIONMAX, LEFT , RIGHT, BOTTOM } from './LightBoxConstant';
...
class ReactImageLightbox extends Component {
  ...
  static parseMouseEvent(mouseEvent) {
    return { id: 'mouse', source: SOURCE_MOUSE, x: parseInt(mouseEvent.clientX, 10), y: parseInt(mouseEvent.clientY, 10) };
  }

  static parseTouchPointer(touchPointer) {
    return { id: touchPointer.identifier, source: SOURCE_TOUCH, x: parseInt(touchPointer.clientX, 10), y: parseInt(touchPointer.clientY, 10) };
  }

  static parsePointerEvent(pointerEvent) {
    return { id: pointerEvent.pointerId, source: SOURCE_POINTER, x: parseInt(pointerEvent.clientX, 10), y: parseInt(pointerEvent.clientY, 10) };
  }

  // Request to transition to the previous image
  static getTransform({ x = 0, y = 0, zoom = 1, width, targetWidth,forms ,rotate, position}) {
    let nextX = x;
    let windowWidth = getWindowWidth();
    if(forms && position != BOTTOM) {
      windowWidth = windowWidth - (0.25 * windowWidth);
    }
    if(rotate == undefined) {
      rotate =0;
    }
  ...
  constructor(props) {
    ...
    this.handleLeftRotationButtonClick = this.handleLeftRotationButtonClick.bind(this);
    this.handleRightRotationButtonClick = this.handleRightRotationButtonClick.bind(this);
  }
  ...
  getBestImageForType(srcType) {
    ...
    } else if(imageSrc && imageSrc.endsWith(".pdf") || this.props.documentType === 'application/pdf') { 
      return {
        src: imageSrc,
        height: 700,
        width: 500,
  ...
  handleTouchMove(event) {
    if (this.shouldHandleEvent(SOURCE_TOUCH)) {
      this.multiPointerMove(event, [].map.call(event.changedTouches, eventTouch => 
  ...
  handleLeftRotationButtonClick() {
    this.setState((prevState) => ({ 
      rotate: ((prevState.rotate + 90) % ROTATIONMAX) 
   }))
  }

  handleRightRotationButtonClick() {
    this.setState((prevState) => ({ 
  ...
  downloadAllFiles(event) {
    this.props.onDownloadRequest(this.props.previousFilesHistory);
  }
  ...
  requestMove(direction, event) {
    ...
    const nextState = { zoomLevel: MIN_ZOOM_LEVEL, offsetX: 0, offsetY: 0 ,rotate:ROTATIONMIN};
  ...
  render() {
    ...
    if (!animationDisabled && this.isAnimating()) {
      transitionStyle = { ...transitionStyle, transition: `transform ${animationDuration}ms` };
    }
    ...
    const addImage = (srcType, imageClass, transforms) => {
      ...
      if (discourageDownloads) {
        ...
        images.push(
          <div className={`${imageClass} ril__image ril__imageDiscourager`} onDoubleClick={this.handleImageDoubleClick} onWheel={this.handleImageMouseWheel} style={imageStyle} key={imageSrc + keyEndings[srcType]} >
            <div className="ril-download-blocker ril__downloadBlocker" />
          </div>
        );
      } else {
        if(imageSrc && imageSrc.endsWith(".pdf") || this.props.documentType === "application/pdf") {
          images.push(
            <object type="application/pdf" className={`${imageClass} ril__image ril__imageDiscourager`} style={imageStyle} key={imageSrc + keyEndings[srcType]} data={imageSrc} height="90%" width="100%">
              <param name="data" value={imageSrc}></param>
            </object>
          );
        } else {
          images.push(
            <img {...(imageCrossOrigin ? { crossOrigin: imageCrossOrigin } : {})} className={`${imageClass} ril__image`}
              onDoubleClick={this.handleImageDoubleClick} onWheel={this.handleImageMouseWheel} 
              onDragStart={e => e.preventDefault()} style={imageStyle} src={imageSrc} key={imageSrc + keyEndings[srcType]} 
              alt={ typeof imageTitle === 'string' ? imageTitle : translate('Image') } draggable={false}
            />
          );
        }
      }
    ...
    addImage('nextSrc', 'ril-image-next ril__imageNext', { x: boxSize.width });
    // Main Image
    addImage('mainSrc', 'ril-image-current', { x: -1 * offsetX, y: -1 * offsetY, zoom: zoomMultiplier ,rotate:this.state.rotate});
    // Previous Image (displayed on the left)
    addImage('prevSrc', 'ril-image-prev ril__imagePrev', { x: -1 * boxSize.width });

    const modalStyle = {
      overlay: { zIndex: 1060, backgroundColor: 'transparent', ...reactModalStyle.overlay },
      content: {
    ...
    return (
      <Modal isOpen onRequestClose={clickOutsideToClose ? this.requestClose : undefined}
        onAfterOpen = {() => {
        ...
        {...reactModalProps}
      >
        <div ref = {this.outerEl} 
          className = {`ril-outer ril__outer ril__outerAnimating ${
            this.props.wrapperClassName
          } ${isClosing ? 'ril-closing ril__outerClosing' : ''}`}
          style = {{ transition: `opacity ${animationDuration}ms`, animationDuration: `${animationDuration}ms`, animationDirection: isClosing ? 'normal' : 'reverse' }}
        ...
        >
          ...
          <div className={`ril-inner ril__inner ${this.props.forms ? (position == LEFT)  ? "custom-forms right-section" : (position == BOTTOM) ? "custom-forms bottom-section": "custom-forms" : '' }`} onClick={clickOutsideToClose ? this.closeIfClickInner : undefined} >
            {images}

            {prevSrc && this.props.forms && (imageIndex != 0) &&
            ...
            {nextSrc && this.props.forms && (imageIndex != prescImages.length-1) &&
            <button type="button" key="next"
              className="ril-next-button ril__navButtons ril__navButtonNext customnext"
          ...
          {/* forms container div */}
          {this.props.forms && <div className={`stacked-images-forms ${(position == LEFT) ? 'left-section' : (position == BOTTOM) ?  "bottom-section" : ""}`}>
            {this.props.displayForms()}
           </div>}

          {/* Move to previous image button if no forms in the popover */}
          {prevSrc && !this.props.forms && (imageIndex != 0) &&
            <button type="button" key="prev"
              className="ril-prev-button ril__navButtons ril__navButtonPrev"
              aria-label={this.props.prevLabel}
          ...
          {/* Move to next image button  if no forms in the popover*/}
          {nextSrc && !this.props.forms  && (imageIndex != prescImages.length-1) &&
            <button type="button" key="next"
          ...
          <div className="ril-toolbar ril__toolbar" >
            <ul className="ril-toolbar-left ril__toolbarSide ril__toolbarLeftSide">
              {this.props.imageTitle && <li className="ril-toolbar__item ril__toolbarItem me-3">
                <p className='mb-0'>{this.props.imageTitle}</p>
              </li>}

              <li className="ril-toolbar__item ril__toolbarItem font-14">
                <span className="ril-toolbar__item__child ril__toolbarItemChild">
                <React.Fragment>
                  <div className="d-inline-block">{imageIndex+1} / {prescImages.length}</div>
                </React.Fragment>
                </span>
              </li>
            ...
            <ul className="ril-toolbar-right ril__toolbarSide ril__toolbarRightSide">
              {toolbarButtons && 
                toolbarButtons.map((button, i) => (
                  <li key={`button_${i + 1}`} className="ril-toolbar__item ril__toolbarItem" >{button}</li>
                ))
              }

              <li className="ril-toolbar__item ril__toolbarItem">
                <button type="button" key="rotate-AntiClockWise"
                  ...
                  disabled={false}
                  onClick={!this.isAnimating() ? this.handleRightRotationButtonClick : undefined}
                />
              </li>

              <li className="ril-toolbar__item ril__toolbarItem">
                <button type="button" key="rotate-ClockWise"
                  aria-label={"rotate-ClockWise icon"}
                  className={[
                    'ril__toolbarItemChild',
                    'ril__builtinButton',
                  ...
                  disabled={false}
                  onClick={!this.isAnimating() ? this.handleLeftRotationButtonClick : undefined}
                />
              </li>
              ...
              {enableZoom &&
                <li className="ril-toolbar__item ril__toolbarItem">
                  <button type="button" key="zoom-out"
              ...
              {enableZoom &&
                <li className="ril-toolbar__item ril__toolbarItem">
                  <button type="button" key="zoom-in"
              ...
              <li className="ril-toolbar__item ril__toolbarItem">
                {this.props.hideDownload ? "" : <a className="ril__toolbarItemChild ril__builtinButton" target="_blank" title="Download Image" href={prescImages[imageIndex]}>
              ...
              {downloadAll && 
                <li className="ril-toolbar__item ril__toolbarItem">
                  <button type="button" key="downloadAll"
                    aria-label={"Download All"}
                    className={[
                      'ril__toolbarItemChild',
                      'ril__builtinButton',
                      'ril__downloadAllButton',
                    ].join(' ')}
              ...
              {/* <li className="ril-toolbar__item ril__toolbarItem">
                { <a className="ril__toolbarItemChild ril__builtinButton" target="_blank" title="Download Image" href="javascript:void(0)">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 16 16">
                    <g id="download_all_icon_16px" transform="translate(-180.258 -387.452)">
                      <rect id="Rectangle_3299" data-name="Rectangle 3299" width="16" height="16" rx="3" transform="translate(180.258 387.452)" fill="none" />
                      <g id="download-all" transform="translate(174.832 380.53)">
                        <path id="Subtraction_119" data-name="Subtraction 119" d="M8.52,12A4.06,4.06,0,0,1,4.93,9.857L4.9,9.788H.961A.946.946,0,0,1,.283,9.5.974.974,0,0,1,0,8.815V.991A1,1,0,0,1,.99,0H4.354a1,1,0,0,1,.766.41l.826,1.258h5.105a1,1,0,0,1,.986.991V5.895l.027.055a.387.387,0,0,1,.031.064A4.065,4.065,0,0,1,8.52,12Zm.01-7.23a3.2,3.2,0,1,0,3.2,3.193A3.2,3.2,0,0,0,8.529,4.77ZM.861.848h0V8.94H4.59l-.033-.152a4.062,4.062,0,0,1,6.414-4.073l.2.152V2.535H6.027a1,1,0,0,1-.834-.452L4.414.862.861.848Z" transform="translate(7.426 8.921)" fill="#fff" />
                        <path id="Path_51588" data-name="Path 51588" d="M55.089,51.564l-.447.462V50.3a.3.3,0,0,0-.3-.3h-.522a.3.3,0,0,0-.3.3V52l-.447-.447a.559.559,0,0,0-.82.76l.03.03,1.4,1.4a.549.549,0,0,0,.79,0l1.4-1.4a.563.563,0,0,0-.03-.79.555.555,0,0,0-.76.015Z" transform="translate(-38.135 -35.066)" fill="#fff" />
                        <path id="Path_51588_-_Outline" data-name="Path 51588 - Outline" d="M53.825,49.875h.522a.426.426,0,0,1,.426.426v1.413l.183-.189v0l.05-.047a.7.7,0,0,1,.48-.193h0a.664.664,0,0,1,.453.177.688.688,0,0,1,.036.971l-1.4,1.4a.677.677,0,0,1-.97,0L52.17,52.4a.685.685,0,0,1,.5-1.156.692.692,0,0,1,.5.218l.226.226V50.3A.426.426,0,0,1,53.825,49.875Zm.692,2.47V50.3a.171.171,0,0,0-.17-.17h-.522a.171.171,0,0,0-.17.17v2.007l-.665-.665a.439.439,0,0,0-.321-.14.424.424,0,0,0-.291.113.431.431,0,0,0-.137.3.424.424,0,0,0,.113.306l.027.027,1.4,1.4a.422.422,0,0,0,.609,0l1.4-1.4a.437.437,0,0,0-.027-.606.409.409,0,0,0-.279-.109h0a.449.449,0,0,0-.3.121Z" transform="translate(-38.137 -35.069)" fill="#fff" />
                      </g>
                    </g>
                  </svg>
                </a > }
          ...
          {this.props.imageCaption &&
            <div onWheel={this.handleCaptionMousewheel} onMouseDown={event => event.stopPropagation()} className="ril-caption ril__caption" ref={this.caption} >
...
ReactImageLightbox.propTypes = {
  ...
  //Position Properties , Default is Right 
